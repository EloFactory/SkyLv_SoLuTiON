    ...
    internal class CastOnDash
    {
        ...
        static void Unit_OnDash(Obj_AI_Base sender, Dash.DashItem args)
        {
            ...
            if (SkyLv_Taric.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
                ...
                {
                    foreach (var AllyHero in ObjectManager.Get<Obj_AI_Hero>().Where(a => !a.IsMe && a.IsDead && a.Team == ObjectManager.Player.Team && Player.Distance(a) < 1600 && (a.HasBuff("TaricWAllyBuff") || a.HasBuff("TaricW"))))
                    {
                        var Allytarget = ObjectManager.Get<Obj_AI_Hero>().Where(t => !t.IsDead && t.Team != ObjectManager.Player.Team && AllyHero.Distance(args.EndPos) < E.Range).FirstOrDefault();
