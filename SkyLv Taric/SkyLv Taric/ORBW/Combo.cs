namespace SkyLv_Taric
{
    ...
    internal class Combo
    {
        ...
        static Combo()
        {
            ...
            foreach (var Ally in ObjectManager.Get<Obj_AI_Hero>().Where(a => a.Team == Player.Team && a.NetworkId != Player.NetworkId))
            {
        ...
        public static void ComboLogic()
        {
            ...
            if (SkyLv_Taric.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
               var target = TargetSelector.GetTarget(E.Range, TargetSelector.DamageType.Physical);
                ...
                {
                    foreach (var AllyHeroE in ObjectManager.Get<Obj_AI_Hero>().Where(a => !a.IsMe && !a.IsDead && a.Team == ObjectManager.Player.Team && Player.Distance(a) < 1600 && (a.HasBuff("TaricWAllyBuff") || a.HasBuff("TaricW"))))
                    {
                        var Allytarget = ObjectManager.Get<Obj_AI_Hero>().Where(t => !t.IsDead && t.Team != ObjectManager.Player.Team && AllyHeroE.Distance(t) < E.Range).FirstOrDefault();

                        if (Allytarget.IsValidTarget())
                        {
                            if (SkyLv_Taric.Menu.Item(AllyHeroE.ChampionName + "AllyCCEComboFromAlly", true).GetValue<bool>() && (AllyHeroE.IsCharmed || AllyHeroE.IsStunned || AllyHeroE.IsRooted || AllyHeroE.IsWindingUp))
                            {
                                E.Cast(Allytarget.ServerPosition, PacketCast);
                                return;
                            }

                            if (SkyLv_Taric.Menu.Item(AllyHeroE.ChampionName + "TargetCCEComboFromAlly", true).GetValue<bool>() && (Allytarget.IsCharmed || Allytarget.IsStunned || Allytarget.IsRooted || Allytarget.IsWindingUp))
                            {
                                E.Cast(Allytarget.ServerPosition, PacketCast);
                                return;
                            }
