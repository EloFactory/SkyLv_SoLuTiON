namespace SkyLv_Tristana
{
    ...
    public static class CustomLib
    {
        ...
        public static Vector3 GetBehindPosition(Obj_AI_Hero target)
        {
            return target.ServerPosition.Extend(GetPushPosition(target), -(300));
        }

        public static Vector3 GetBehindPositionExtend(Obj_AI_Hero target)
        {
            return target.ServerPosition.Extend(GetPushPosition(target), -(1000));
        }

        public static Vector3 GetPushPosition(Obj_AI_Hero target)
        {
                    ...
                    else
                    {
                        var hero =
                            SDK.GameObjects.AllyHeroes.Where(
                                ...
                                .MaxOrDefault(i => new LeagueSharp.SDK.Modes.Priority().GetDefaultPriority(i));
                        if (hero != null)
                        {
